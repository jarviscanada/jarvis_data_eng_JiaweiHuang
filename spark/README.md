# London Gift Shop — RFM Analytics (PySpark)
...
- **Databricks** for cloud-based distributed data processing and notebook environment
- **PySpark** for scalable data ingestion, complex aggregations, and RFM scoring across large transaction volumes
...
orders and inventory. This data is stored in a CSV file, ingested into Databricks, and processed through our
distributed analytics engine to generate insights that are then fed back into marketing automation tools.


### Flow
- **Source:** LGS Web App captures transactional events.
- **Storage:** Transactions are recorded in the LGS Database and ingested into Databricks as a managed Delta table.
- **Analytics:** PySpark extracts data to perform cleaning (handling cancellations) and RFM scoring modelling.
...
- Leverage Databricks' scalability to process the full transaction history in real-time, enabling live RFM score updates
as new orders arrive rather than periodic batch analysis.
